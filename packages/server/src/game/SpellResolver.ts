import { PlayerState, SpellSlot } from "./GameState";
import { SPELLBOOK, SpellId, SpellResult, GAME } from "shared";
import { ActiveEffects } from "./EffectEngine";
...
export class SpellResolver {
    ...
    for (let i = 0; i < caster.slots.length; i++) {
      ...
      switch (spellId) {
        ...
        case "W": { // Wyra (Water): Restore mana. Base 25 at 100% acc, scales to 8 at 0%
          ...
          caster.mana = Math.min(GAME.MANA_MAX, caster.mana + manaRestored);
        ...
        case "R": { // Rhael (Air): Restore HP. 15 HP at 100% acc, scales to 4 at 0%
          ...
          caster.hp = Math.min(GAME.HP_MAX, caster.hp + hpHealed);
  ...
  private static applyDamage(target: PlayerState, amount: number): number {
    if (target.shield > 0) {
